...
public class DispatcherHelper {
            ...
            try {
                ...
                if (bean.getClass().isAnnotationPresent(RestActionBean.class)) {
                    ...
                    return new ErrorResolution(HttpServletResponse.SC_NOT_FOUND, "The requested handler method "
                                                                                 + "(" + eventName + ") is not found on this RestActionBean (" + ctx1.getActionBean().getClass().getSimpleName() + ")");
                } else {
                    throw sse;
                }
            }
    ...
    public static Method[] findCustomValidationMethods(Class<? extends ActionBean> type) {
        ...
        if (validations == null) {
            ...
            SortedSet<Method> validationMethods = new TreeSet<>((o1, o2) -> {
                // If one of the methods overrides the others, return equal!
                if (o1.getName()
                            .equals(o2.getName())
                    && Arrays.equals(o1.getParameterTypes(),
                ...
                int returnValue = Integer.compare(ann1.priority(),
                                                  ann2.priority());

                if (returnValue == 0) {
                    returnValue = o1.getName()
            ...
            customValidations.put(type,
                                  new WeakReference<>(validations));
        }
        ...
        if (doValidate) {
            ...
            if (bean.getClass().isAnnotationPresent(RestActionBean.class)) {
                ...
                if (validationErrors != null && !validationErrors.isEmpty()) {
                    ...
                    Map< Object, Object> jsonErrorMap = new HashMap<>();

                    // First, append the global errors -- if any
                    List< String> jsonGlobalErrors = new ArrayList<>();
                    ...
                    ArrayList< Map< String, Object>> allFieldErrors = new ArrayList<>();
    ...
    public static Resolution invokeEventHandler(ExecutionContext ctx) throws Exception {
        ...
        return ctx.wrap(ctx1 -> {
            ...
            String requestMethod = ctx1.getActionBeanContext().getRequest().getMethod();

            if (!handler.getName().equalsIgnoreCase(requestMethod)) {
                // Check the HTTP request method and ensure that the target handler
                // method supports it.
                Collection<HttpRequestMethod> supportedRequestMethods = new ArrayList<>();
                ...
                if (!methodSupported) {
                    String msg = "The handler method [" + handler.getName() + "] in ActionBean ["
                                 ...
                                 + ctx1.getActionBeanContext().getEventName() + "] does not support "
                    ...
                    log.debug("Invoking the handler method [" + handler.getName() + "] in ActionBean ["
                              + bean.getClass().getName() + "] for eventName ["
                              + ctx1.getActionBeanContext().getEventName() + "] with request method [" + requestMethod + "]");
                }
            }
            ...
            if (NameBasedActionResolver.isAsyncEventHandler(handler)) {
                ActionBeanContext abc = ctx1.getActionBeanContext();
    ...
    public static void executeResolution(ExecutionContext ctx, Resolution resolution) throws Exception {
        ...
        Resolution retval = ctx.wrap(context -> {
