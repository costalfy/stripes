...
public class ObjectFactoryTests extends StripesTestFixture {
    ...
    public void instantiateClasses(ObjectFactory factory, Class<?>... classes) {
        for (Class<?> clazz : classes) {
            ...
            Assertions.assertSame(o.getClass(),
                                  clazz);
        }
    }
    ...
    public void instantiateInterfaces(ObjectFactory factory, Class<?>... classes) {
        for (Class<?> clazz : classes) {
            log.debug("Instantiating ",
                      clazz);
            Object o = factory.newInstance(clazz);
            log.debug("Implementation class is ",
                      o.getClass()
                              .getName());
            Assertions.assertNotNull(o);
            Assertions.assertTrue(clazz.isAssignableFrom(o.getClass()));
        }
    }
    ...
    public void constructor() {
        ...
        ConstructorWrapper<Adder> constructor = getDefaultConfiguration().getObjectFactory()
                .constructor(Adder.class,
                             Integer.TYPE,
                             Integer.TYPE);

        int a = 37, b = 91;
        Adder adder = constructor.newInstance(a,
                                              b);
        Assertions.assertNotNull(adder);
        Assertions.assertSame(adder.getClass(),
                              Adder.class);
        Assertions.assertEquals(adder.sum(),
                                a + b);
    }
    ...
    public void missingInterfaceImpl() throws Throwable {
        Assertions.assertThrows(StripesRuntimeException.class,
                                () -> {
                                    log.debug("Attempting to instantiate ",
                                              Runnable.class,
                                              " expecting failure");
                                    getDefaultConfiguration().getObjectFactory()
    ...
    public void customInterfaceImpl() {
        ...
        instantiateInterfaces(factory,
                              CharSequence.class,
                              List.class,
                              Runnable.class);
        Assertions.assertSame(factory.newInstance(List.class)
                                      .getClass(),
                              LinkedList.class);
    }
    ...
    public void multipleSequentialPostProcessors() {
        ...
        log.debug("Got ",
                  buf);
        Assertions.assertEquals(counter.intValue(),
                                5);
    }
}
